...
public class Endpoints : IEndpointsOrLink
{
    ...
    [JsonIgnore]
    public Dictionary<string, Uri>? ExtensionUris => this.ExtensionData?.Where(kv => kv.Value.ValueKind is JsonValueKind.String && Uri.TryCreate(kv.Value.Deserialize<string>(), new(), out Uri? _)).Select(kv => (key: kv.Key, value: kv.Value.Deserialize<Uri>()!)).ToDictionary(kv => kv.key, kv => kv.value);
}
